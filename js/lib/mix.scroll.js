...
define(['dom'], function(dom) {
    function Scroll(sel, options) {
        ...
        that.scroller = dom.$('.scroller', that.wrapper) || that.wrapper.children[0];
    ...
    Scroll.prototype = {
        ...
        _start: function(e) {
            ...
            if (that.options.momentum) {
                ...
                if (x != that.x || y != that.y) {
                    if (that.options.useTransition) {
                        that._unbind(Mix.event.TRNEND_EV);
                    } else {
            ...
            that._bind(Mix.event.MOVE_EV);
            that._bind(Mix.event.END_EV);
            that._bind(Mix.event.CANCEL_EV);
        },
        _move: function(e) {
            var that = this,
                point = new Mix.event.Event(e),
        ...
        _end: function(e) {
            ...
            that._unbind(Mix.event.MOVE_EV);
            that._unbind(Mix.event.END_EV);
            that._unbind(Mix.event.CANCEL_EV);
        ...
        _startAni: function() {
            ...
            if (that.options.useTransition) {
                ...
                that.animating = false;
                if (step.time) that._bind(Mix.event.TRNEND_EV);
        ...
        destroy: function() {
            ...
            that._unbind(Mix.event.START_EV);
            that._unbind(Mix.event.MOVE_EV);
            that._unbind(Mix.event.END_EV);
            that._unbind(Mix.event.CANCEL_EV);

            if (that.options.useTransition) that._unbind(Mix.event.TRNEND_EV);
        ...
        disable: function() {
            ...
            this._unbind(Mix.event.MOVE_EV);
            this._unbind(Mix.event.END_EV);
            this._unbind(Mix.event.CANCEL_EV);
        },
        enable: function() {
            this.enabled = true;
        },
