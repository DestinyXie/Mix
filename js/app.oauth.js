...
define(['X'], function(X) {
    Mix.auth.qq = {
        ...
        succCall: function() {
            Mix.auth.qq.qqLogInfo = Mix.auth.qq.getLoginInfo();
            Mix.auth.qq.checkAuth();
        },
        onOAuth: function(winNam, url) {
            var that = Mix.auth.qq;
    ...
    Mix.auth.sina = {
        ...
        succCall: function() {
            Mix.auth.sina.sinaLogInfo = Mix.auth.sina.getLoginInfo();
            Mix.auth.sina.checkAuth();
        },
        onOAuth: function(winNam, url) {
            var that = Mix.auth.sina;
            if (winNam == that.oauthWindow && url.indexOf(that.backUrl) == 0) {
    ...
    Mix.auth.renren = {
        ...
        succCall: function() {
            Mix.auth.renren.renrenLogInfo = Mix.auth.renren.getLoginInfo();
            alert(JSON.stringify(Mix.auth.renren.renrenLogInfo));
            Mix.auth.renren.checkAuth();
        },
        onOAuth: function(winNam, url) {
            var that = Mix.auth.renren;
            if (winNam == that.oauthWindow && url.indexOf(that.backUrl) == 0) {
                //fix：SINA会回调2次，设置1S内只
                if (that.lastCall && new Date() - that.lastCall < 1000) return;
                var par = url.split('?')[1],
